...
No native application currently ships from this repository. These notes define
future Kotlin and Swift adapter behavior only; they do not add native screens,
production flows, or a shared-code generation pipeline.
...
`mergeRemoteMessage`, `duplicateClientRequestIdReconciliation`, `mergeReadState`, `mergeReadStateIgnoresEarlierTimestamp`, `unreadCount`,
`composerSelectionMutualExclusion`, `deleteTombstoneRestoresAfterFailure`,
...
observable model behind platform-native design tokens and the FISH calm UI rules. Production Android/iOS chat screens, generated shared-code pipelines,
