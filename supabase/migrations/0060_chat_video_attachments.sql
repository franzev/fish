-- Bounded MP4 video attachments reuse the private file attachment pipeline.
-- Video remains kind = 'file' so existing clients safely render it as a file
-- until they understand the video MIME type.

update storage.buckets
set file_size_limit = 26214400,
    allowed_mime_types = array[
      'image/webp', 'image/jpeg', 'audio/mp4', 'video/mp4', 'application/pdf', 'text/plain', 'text/csv',
...
alter table public.message_attachments
  drop constraint if exists message_attachments_source_byte_size_check,
  drop constraint if exists message_attachments_stored_byte_size_check;
...
alter table public.message_attachments
  add constraint message_attachments_source_byte_size_check check (
    source_byte_size >= 1 and (
      ...
      or (kind = 'file' and (
        (source_mime_type = 'video/mp4' and source_byte_size <= 26214400)
        or (source_mime_type <> 'video/mp4' and source_byte_size <= 10485760)
      ))
    )
  ),
  add constraint message_attachments_stored_byte_size_check check (
    stored_byte_size is null or stored_byte_size between 1 and
      case when stored_mime_type = 'video/mp4' then 26214400 else 10485760 end
  );
...
begin
  ...
  if (v_kind = 'image' and p_source_byte_size not between 1 and 26214400)
    or (v_kind = 'file' and p_source_byte_size not between 1 and
      case when p_source_mime_type = 'video/mp4' then 26214400 else 10485760 end)
