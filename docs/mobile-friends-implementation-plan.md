...
Status: Implemented (2026-07-30) — all six tasks landed (21 commits, `092df133..9fae60cb`); every task passed a spec-compliance and a code-quality review with fix loops. Platform order: Android first. Scope: the existing native direct-chat apps only.
...
## As-built notes (what changed against this plan during execution)

- **Review-driven hardening beyond the plan, both platforms:** supabase-kt's functions client throws on non-2xx before any manual status check, so Android's `{code, error}` parsing had to translate `RestException` (`d46de7ef`) — without it every server calm string and all three conflict mappings were dead code. Both platforms' `friends:user:{id}` streams gained reconnect loops with a client-raised `StreamResumed` reason that also refreshes the directory (backfilling accepts that land inside an outage); iOS additionally survives SDK-internal rejoins, server-side channel closes, and access-token expiry (`619ade92`, `b4573876`), with the loop's decision logic extracted to pure tested seams on both platforms.
- **Directory refresh:** Android coalesces a refresh arriving mid-fetch into exactly one trailing pass (`eb5140c2`); `refreshDirectory()` is the single-flight public entry on both platforms.
- **Reachability tightened:** the Android conversation-list relaxation is `friendsEnabled && role == Client`, not flag-only — a coach gains no new destination (`5ea10a57`). The requests-waiting row also renders on the zero-conversation empty state (it was otherwise unreachable at 0 conversations) and survives a directory-failure notice on both platforms (`4948f419`, `9fae60cb`).
- **Structural divergences:** Android's `feature/friends` uses one `FriendsViewModel` + injected `FriendsNotices` (Context-free) instead of the two-model split; iOS follows the two-model split with fresh models per presentation and a presentation-token guard on the requests sheet's self-dismissal (`5a5008d4`). iOS `FriendsData` owns `FriendsBackendConfiguration` and its own realtime client (ChatData internals aren't importable). The `friend-command` payloads/statuses are branch-on-constants on iOS (`FriendCommandCode`/`FriendRequestStatus`) and literals-like-web on Android (module-boundary guard forbids importing the wire constants).
- **Guard/registry upkeep:** `scripts/ios-token-guard.mjs` now enforces import boundaries for `FriendsData`/`Friends` (and holds `PresenceData` to the adapters-only Supabase rule it always claimed); the parity registry pairs the friends surfaces cross-platform by entry with honest `propsBreak` classifications.
- **Deferred edge accepted symmetrically:** realtime events refresh the count and directory but do not reload an already-open requests list (a naive reload would race the in-flight respond); every open/reopen reloads.

**Remaining (external, rides the release-verification track — see `apps/android/README.md` and `apps/ios/App/README.md` for the recorded steps):** the 7-step two-device manual matrix (send/accept/decline/crossed requests/coach/flag-off, plus the iPad-rail check) against a project with Edge `FRIENDS_ENABLED=true`; the physical-device pass; `pnpm android:instrumented` on an emulator (written and previously green, not re-run in the final headless review). Enabling the feature is a product decision: Edge env `FRIENDS_ENABLED=true` plus `FISH_ANDROID_FRIENDS_ENABLED=true` / `FISH_FRIENDS_ENABLED=true` at build time; both apps ship dark and fail closed until then.
...
- [x] **1.1 Create the module** with the `data/presence` build file as the template (`api(libs.kotlinx.coroutines.core)`, `implementation(project(":core:supabase"))`, supabase-kt postgrest/functions/realtime, serialization, ktor-client-mock for tests). Register in `settings.gradle.kts`.

- [x] **1.2 Domain models** in `FriendsModels.kt`:
...
- [x] **2.1 Icon.** Add `FishIcons.PersonAdd` (Tabler `user-plus` outline path, hand-authored `ImageVector` like the existing 33) and sync `design/parity/native-components.json` per its schema. Run `pnpm android:verify-design`.

- [x] **2.2 UI state + ViewModel.** UI state in `model/FriendsModels.kt`:
...
- [x] **4.2 Wire + REST adapter.** `FriendsWire.swift`: snake_case `CodingKeys` structs with `.domain` (candidate jsonb, request rows, `{code, error}` failures, camelCase-encoding command bodies and the `{request: {...}}` response). Unknown status or nil profile → `.unavailable`. `RestFriendDirectory` hand-rolls `rest/v1/rpc/{name}` exactly like `RestConversationDirectory.rpc` (`RestConversationDirectory.swift:42-81`), including the 15s timeout and `chatFailure`-style non-2xx mapping; `count_incoming_friend_requests` decodes a bare integer body.

- [x] **4.3 Edge commands.** `EdgeFunctionFriendCommands` per the `EdgeFunctionAttachmentCommands.send` template (`EdgeFunctionAttachmentCommands.swift:105-145`) against `functions/v1/friend-command`, bodies `{action: "send-request", targetId, clientRequestId}` / `{action: "respond-request", requestId, response}`; non-2xx decodes `{code, error}` into `FriendCommandFailure` so callers can branch on `failure.code`.

- [x] **4.4 Events.** `FriendsLive.events(userId:)` — one private broadcast channel `friends:user:{userId}`, event `friends.changed`, per the `attentionEvents` template (`ChatLive.swift:300-340`), decoding reason strings to `FriendEventReason` (unknown → `.unknown`, still emitted: every event triggers a refetch anyway).

- [x] **4.5 Tests** (swift-testing): wire decode fixtures for all statuses/rows/failures; `RestFriendDirectory` + `EdgeFunctionFriendCommands` against `StubURLProtocol` (success, `{code, error}` calm mapping, 401 → not-authenticated, scalar count decode); command body encode assertions (action literals, camelCase). Run `pnpm ios:test`.

- [x] **4.6 Commit** `feat(ios): add FriendsData module for search, requests, and friend events`.
...
- [x] **6.6 Verify end to end** on the simulator against dev Supabase with the Edge `FRIENDS_ENABLED=true`: the same two-device matrix as Task 3.7 (search/send/accept/decline, conversation appears both sides without relaunch, coach sees nothing, flag-off build unchanged), plus iPad two-pane rail entry mid-conversation. `pnpm ios:test`, `pnpm ios:tokens:check`, `pnpm ios:guard`, `pnpm build`.

- [x] **6.7 Commit** `feat(ios): wire friends entry points, sheets, and directory refresh`.
