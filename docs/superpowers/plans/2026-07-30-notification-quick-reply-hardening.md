...
A second follow-up commit ("fix(ios): rebuild the drain snapshot on every pass") moves the directory snapshot and drainer construction inside the repeat loop — pass 1 refreshes the directory, so pass 2 judging fresh replies against the pre-refresh snapshot silently destroyed replies for newly-authorized conversations. The owner-id freeze stays outside the loop (the sign-out guard compares against the account that started the drain), and the failure-notice closure uses the model's injected `application` rather than `UIApplication.shared`.

Accepted residuals recorded during this review: iOS has no background owner for the composer text outbox (Android's worker flushes it; on iOS a queued send whose conversation is closed waits for that conversation to reopen — the largest remaining platform divergence, deferred as its own future slice); appending to a live `ConversationStore.draft` fires its `didSet` typing signal, so the other participant briefly sees "typing…" from a background drain — a pre-existing programmatic-draft wart (two other call sites do the same) whose fix is a FishKit `appendDraft` that persists without scheduling typing, deferred as its own slice; a draft saved for a `conversation_not_available` failure lands in a conversation whose notice tap cannot open it; the badge can stay stale until the next refresh when nothing sent; mark-read re-fires per drain pass for stuck entries (server-monotonic, paced); notice copy is a hardcoded literal (no string catalog exists in the App target).

The final integration review then produced one more commit ("fix(ios): never drop replies on an untrusted directory"): `ConversationDirectoryStore` reports `.failed` — an empty conversation list — after a thrown refresh, and the drain's guard admitted it, so an offline cold-start drain judged every reply against an empty authorization set and silently destroyed them. The drain now requires `phase == .ready` (outer guard and per-pass), and the drainer's unauthorized branch preserves the text as a confirmed draft before removing (keeping the entry when preservation fails) instead of bare removal — same-account "not in the directory" is ambiguous between revocation and a stale cache, sign-out clears the store, and the owner guard covers account switches, so nothing needed destruction to stay safe. Integration-review residuals, accepted: the failure notice is not withdrawn when its conversation is later opened (both platforms; it collapses per conversation and auto-cancels on tap); Android's degraded failure notice (no message id, e.g. pre-migration entries) opens the app root while its copy says "the conversation"; Android's worker completion remains coupled to the whole text outbox (already recorded at Task 6).

**Tracked follow-up (the one remaining destroy path, ship-approved but named):** Android's `listAuthorizedConversations` falls back to the Room cache on remote failure and still reports `Success`, so the drain's `allowed` set can be stale-but-trusted — a reply to a brand-new conversation not yet cached, drained while the server is unreachable, is removed with a generic notice and no draft (`appendDraft` also fails there: no Room row). Lower blast radius than the fixed iOS twin (two-condition coincidence, one entry) and strictly better than the pre-plan behavior, but it is now the only place the feature destroys user-typed text. Fix shape: surface the cache-fallback as non-authoritative (flag on `AuthorizedChatDirectory` or a drain-only variant) and return `Retry` with the entry intact when the read wasn't authoritative.
...
5. iOS: a reply whose send is rejected (401/403/`conversation_not_available`) → "Your reply didn’t send" once per conversation with the text surviving as the composer draft; a reply to a conversation missing from the local directory is preserved as a draft with no notice; a failed directory read keeps replies queued.
