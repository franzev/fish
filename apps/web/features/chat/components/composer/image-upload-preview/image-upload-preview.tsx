...
export function ImageUploadPreview({ images, onRemove, onRetry }: ImageUploadPreviewProps) {
  ...
  return (
    <ul aria-label="Files to send" className="flex flex-wrap justify-start gap-xs px-xs pt-xs">
      {images.map((image) => (
        <li key={image.clientUploadId} className="w-chat-image-preview shrink-0">
          <div className={cn(
            ...
            {image.kind === "image" ? (
              ...
              <img src={image.previewUrl} alt="Preview of image to send" className="size-full object-cover" />
            ) : (
            ...
            {image.status !== "ready" && image.status !== "failed" && (
              <span className="absolute inset-0 flex items-center justify-center bg-scrim">
                <CircularProgress
                  ...
                  label={`${statusLabel[image.status]} file`}
