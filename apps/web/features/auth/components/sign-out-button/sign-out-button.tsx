...
/* The Profile settings "Sign out" row (the header now uses UserMenu instead).
   ...
   a primary. Routes through the shared useSignOut hook so clearChatStore()
   (CR-01) always runs before the redirect, same as UserMenu's Sign out item.
...
export function SignOutButton({ className }: SignOutButtonProps = {}) {
  const { signOut, loading, notice } = useSignOut();
